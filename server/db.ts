...
import { InsertUser, users, professionals, appointments, availability, InsertProfessional, InsertAppointment, InsertAvailability } from "../drizzle/schema";
...
export async function getAppointmentsByProfessionalId(professionalId: number) {
  const db = await getDb();
  if (!db) return [];

  return db.select().from(appointments).where(eq(appointments.professionalId, professionalId));
}
...
export async function getAvailabilityByProfessionalId(professionalId: number) {
  const db = await getDb();
  if (!db) return [];

  return db.select().from(availability).where(eq(availability.professionalId, professionalId));
}
